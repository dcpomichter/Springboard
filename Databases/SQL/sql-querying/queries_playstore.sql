...
SELECT * FROM analytics WHERE NOT rating IS NULL and min_installs<50 ORDER BY rating DESC;
...
SELECT app_name, last_updated FROM analytics ORDER BY (CURRENT_DATE - last_updated)/ 365 DESC LIMIT 1;
--12. Find the most expensive app (the query is very similar to #11).
SELECT app_name, price FROM analytics ORDER BY price DESC LIMIT 1;
--13. Count all the reviews in the Google Play Store.
SELECT COUNT(reviews) AS total_reviews FROM analytics;
--14. Find all the categories that have more than 300 apps in them.
SELECT category, COUNT(id) as total_apps FROM analytics  GROUP BY category HAVING COUNT(id)>300;
...
SELECT app_name, reviews, min_installs, (reviews / min_installs) AS review_to_install FROM analytics WHERE min_installs>=100000 ORDER BY review_to_install DESC LIMIT 1;
